...
st.markdown('<p class="main-title">📊 Relatórios de Veículos Elétricos</p>', unsafe_allow_html=True)
st.markdown('<p class="subtitle">Geração de Relatórios Personalizados por Filtros</p>', unsafe_allow_html=True)
...
# ------------------- FILTROS -------------------
regioes_df = carregar("SELECT DISTINCT regiao, id_regiao FROM regiao ORDER BY regiao")
regioes = adicionar_opcao_todos(regioes_df['regiao'].tolist())
regioes_sel = st.sidebar.multiselect("🌎 Regiões", regioes, default=["Todos"])

if regioes_sel and "Todos" not in regioes_sel:
    ids_regioes = regioes_df[regioes_df['regiao'].isin(regioes_sel)]['id_regiao'].tolist()
    estados_df = carregar(f"SELECT uf, id_estado FROM estado WHERE id_regiao IN {sql_in_list(ids_regioes)} ORDER BY uf")
else:
    estados_df = carregar("SELECT uf, id_estado FROM estado ORDER BY uf")
estados = adicionar_opcao_todos(estados_df['uf'].tolist())
estados_sel = st.sidebar.multiselect("🏙️ Estados", estados, default=["Todos"])

if estados_sel and "Todos" not in estados_sel:
    ids_estados = estados_df[estados_df['uf'].isin(estados_sel)]['id_estado'].tolist()
    cidades_df = carregar(f"SELECT cidade, id_cidade FROM cidade WHERE id_estado IN {sql_in_list(ids_estados)} ORDER BY cidade")
else:
    cidades_df = carregar("SELECT cidade, id_cidade FROM cidade ORDER BY cidade")
cidades = adicionar_opcao_todos(cidades_df['cidade'].tolist())
cidades_sel = st.sidebar.multiselect("🌆 Cidades", cidades, default=["Todos"])

anos_df = carregar("SELECT DISTINCT ano_inicial AS ano FROM cidade_tipo_modelo ORDER BY ano_inicial")
anos = adicionar_opcao_todos(anos_df['ano'].astype(str).tolist())
anos_sel = st.sidebar.multiselect("📅 Ano", anos, default=["Todos"])

tecnologias_df = carregar("SELECT DISTINCT tecnologia, id_tecnologia FROM tecnologia ORDER BY tecnologia")
tecnologias = adicionar_opcao_todos(tecnologias_df['tecnologia'].tolist())
tecnologias_sel = st.sidebar.multiselect("⚡ Tecnologia", tecnologias, default=["Todos"])

classificacao_df = carregar("SELECT DISTINCT classificacao, id_classificacao FROM classificacao_veiculo ORDER BY classificacao")
classificacoes = adicionar_opcao_todos(classificacao_df['classificacao'].tolist())
classificacoes_sel = st.sidebar.multiselect("🚘 Classificação", classificacoes, default=["Todos"])
...
if pesquisar:
    ...
    if not df.empty:
        ...
        st.subheader(f"🔎 Resultados: {' | '.join(filtros_txt) if filtros_txt else '(sem filtros)'}")
