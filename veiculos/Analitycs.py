...
@st.cache_resource
def get_conn():
    """Cria conexão segura com o banco MySQL na Aiven."""
    caminho_ca = os.path.abspath(st.secrets["SSL_CA_PATH"])  # Ex: "ca.pem"
...
def adicionar_opcao_todos(lista):
    """Adiciona opção 'Todos' no topo de uma lista."""
    return ["Todos"] + lista

def sql_in_list(int_list):
    """Converte lista de IDs em string SQL segura."""
    if not int_list:
        return "(0)"  # retorna ID inexistente
...
# Estados dependem da seleção de regiões
...
btn_pesquisar = col_btn1.button("🔍 Pesquisar")
btn_limpar = col_btn2.button("🧹 Limpar Filtros")
...
if btn_pesquisar:
    ...
    if df.empty:
        ...
        col3.metric("⚙️ Tecnologia Mais Popular", top_tecnologia)

        st.subheader("Visualizações")

        df_cidade = df.groupby('cidade', as_index=False)['quantidade'].sum()
        fig1 = px.bar(df_cidade, x='cidade', y='quantidade', color='cidade',
                      title="Total de Veículos por Cidade")
        st.plotly_chart(fig1, use_container_width=True)

        df_modelo = df.groupby('modelo', as_index=False)['quantidade'].sum()
        fig2 = px.bar(df_modelo, x='modelo', y='quantidade', color='modelo',
                      title="Total de Veículos por Modelo")
        st.plotly_chart(fig2, use_container_width=True)

        df_tec = df.groupby('tecnologia', as_index=False)['quantidade'].sum()
        fig3 = px.pie(df_tec, names='tecnologia', values='quantidade',
                      title="Distribuição por Tecnologia")
        st.plotly_chart(fig3, use_container_width=True)

        df_class = df.groupby('classificacao', as_index=False)['quantidade'].sum()
        fig4 = px.pie(df_class, names='classificacao', values='quantidade',
