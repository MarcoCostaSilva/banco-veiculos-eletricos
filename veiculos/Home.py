...
with col1:
    if st.button("📊 **Analytics**", key="analytics", help="Gráficos e análises detalhadas"):
        st.switch_page("Analytics.py")

with col2:
    if st.button("🗺️ **Dashboard**", key="dashboard", help="Mapa interativo do Brasil"):
        st.switch_page("Dashboard.py")

with col3:
    if st.button("📋 **Relatórios**", key="relatorios", help="Downloads CSV/Excel/PDF"):
        st.switch_page("Relatorios.py")
...
# ------------------- Seção: Sessões do Sistema -------------------
st.markdown("""
    <div class="card">
        <p class="card-title">📊 Sessões do Sistema</p>
        <p class="card-content">
            🗺️ <strong>Dashboard:</strong> Análise espacial e distribuição da frota por estado<br>
            📈 <strong>Analytics:</strong> Filtros avançados, indicadores e gráficos comparativos<br>
            📄 <strong>Relatórios:</strong> Visualizações detalhadas, tabelas e resumo estatístico
        </p>
    </div>
""", unsafe_allow_html=True)

# ------------------- Seção: Tipos de Veículos -------------------
st.markdown("""
    <div class="card">
...
st.markdown("---")
st.markdown("<p style='text-align: center; color: #4a7856;'><strong>Desenvolvido com ❤️ por FMU - Iniciação Científica</strong></p>", unsafe_allow_html=True)